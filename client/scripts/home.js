...
(function(){
    ...
    function getContentPage() {
        partialLoading(allAssetCategories).then(function(categories) {
            populateDataForMultipleBands(categories).then(function(categoryList){
                ...
                createRailsCarousel(assetsByCategory);
                pageNumber += 1;
    ...
    function createBannerCarousel(banners) {
        var owlItemTag = "";
        var owlItems = []
        for(var i=0;i<banners.length;i++) {
            if(i == 0) {
                owlItemTag = owlItemTag + '<div class="item banner_item"><img src="'+ banners[i].image_xdr+'"></div>'                
            } else {
                owlItemTag = owlItemTag + '<div class="item banner_item"><img src="'+ banners[i].image_xdr+'"></div>'                                
            }
        }
        $(".banner").append(owlItemTag);
        initBannerOwlCarousel()
    }

    function createRailsCarousel(assetsByCategory) {
        for(var i=0;i<assetsByCategory;i++) {
            var assetCategory = assetsByCategory[i];
            if(assetCategory.show_rail && ( assetCategory.displayOptions.template=='portrait' || assetCategory.displayOptions.template=='landscape' || assetCategory.displayOptions.type=='googleAd' )) {
                if((assetCategory && assetCategory.display_data && assetCategory.display_data.length>0)) {
                    var items = '<div class="section rails-details"><div class="section app-rails"><div class="title-section"><h2>Watch Now</h2><span>See All</span></div><div class="owl-carousel load owl-theme"><div class="item"> <ul><li> <img src="" alt="sony liv"></li><li> <img src="" alt="sony liv"></li></ul></div></div></div></div>'
                }
            }
        }
    }

    function initBannerOwlCarousel() {
        $('.banner').owlCarousel({
